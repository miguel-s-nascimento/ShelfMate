...
class ExtractBookToAdd(Runnable):
    def __init__(self, llm, memory=False): 
        ...
        prompt_template = PromptTemplate(
            ...
            Your task is to identify from the user input the title and the rating of the book from the user input.
            If the user does not specify the rating, set it to 0.
...
class AddBookReadListChain(Runnable): 
    ...
    def invoke(self, user_input, config): 
        ...
        book_info = self.extract_chain.invoke(user_input) 
        if book_info.rating == 0:
